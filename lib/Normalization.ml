...
module Make(In:UTFString)(Out:UTFString) = struct
  let decompose allowed_tags str =
    let rec insert c1 l =
      ...
      | [] -> [c1]
      | c2::l' as l ->
         let i1 = CharInfo.get_char_descr c1 in
         let i2 = CharInfo.get_char_descr c2 in
         let cc1 = Types.combining_class_to_int (i1.combining_class) in
         let cc2 = Types.combining_class_to_int (i2.combining_class) in
         if cc1 = 0 || cc2 = 0 then c1::l
         else if cc1 < cc2 then c2 :: insert c1 l' else c1 :: l
    in
    let rec fn acc c =
      let i = CharInfo.get_char_descr c in
  ...
  let recompose str =
    ...
    let starter c = (CharInfo.get_char_descr c).combining_class = Starter in

    let same_cc c1 c2 =
      (CharInfo.get_char_descr c1).combining_class =
        (CharInfo.get_char_descr c2).combining_class
    in
    ...
    let rec fn acc l =
      ...
      | x::ls ->
         let rec gn (Node tbl) x ls =
           ...
           | (_, [])       -> raise Not_found
         in
         try iter_commutable (gn (Lazy.force CharInfo.prefix_tree)) l
  ...
  let custom_nfd = decompose

  let custom_nfc allowed_tags str = recompose (decompose allowed_tags str)
end

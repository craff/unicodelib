...
module Make(In:UTFString)(Out:UTFString) = struct
  let decompose allowed_tags str =
    let rec insert c1 l =
      ...
      | c2::l' as l ->
         ...
         let cc1 = CharInfo.combining_class_to_int (i1.combining_class) in
         let cc2 = CharInfo.combining_class_to_int (i2.combining_class) in
  ...
  let recompose str =
    ...
    let starter c = (get_char_descr c).combining_class = Starter in

    let same_cc c1 c2 =
      (get_char_descr c1).combining_class =
        (get_char_descr c2).combining_class
    in
    ...
    let rec fn acc l =
      ...
      | x::ls ->
         let rec gn (Node tbl) x ls =
           ...
           | (_, [])       -> raise Not_found
         in
         try iter_commutable (gn (Lazy.force prefix_tree)) l
